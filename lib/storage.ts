export type UserProfile = { name: string; location: string };
...
export function loadAI(): any | null {
  if (typeof window === "undefined") return null;
  try {
...
export function saveAI(data: any) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(AI_KEY, JSON.stringify(data));
}

export function saveImageBase64(b64: string) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(IMAGE_KEY, b64);
}

export function loadImageBase64(): string | null {
  if (typeof window === "undefined") return null;
