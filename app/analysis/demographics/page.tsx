...
function ArcMeter({ value = 0.0 }: { value?: number }) {
  const size = 720;
  const stroke = 18;
...
export default function DemographicsPage() {
  ...
  const { raceList, ageList, genderList } = useMemo(() => {
    const race: Scores = ai?.data?.race ?? {};
    const age: Scores = ai?.data?.age ?? {};
    const gender: Scores = ai?.data?.gender ?? {};
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        <div style={{ paddingTop: 180, maxWidth: 980, margin: "0 auto", paddingLeft: 28, paddingRight: 28 }}>
          ...
          <button
            type="button"
            onClick={() => router.push("/analysis/image")}
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="dem-bottom-nav">
        ...
        <DiamondButton label="HOME" variant="white" onClick={() => router.push("/")} />
      </div>
    </div>
  );
}
